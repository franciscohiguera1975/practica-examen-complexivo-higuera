...
import { type ProductionOrder, listProductionOrdersAdminApi, createProductionOrderApi, updateProductionOrderApi, deleteProductionOrderApi } from "../api/production_orders.api";
...
export default function AdminProductionOrdersPage() {
  ...
  const save = async () => {
    try {
      ...
      if (!product_name.trim()) return setError("El nombre del producto es requerido");
      ...
      <Paper sx={{ p: 3 }}>
        ...
        <Stack spacing={2} sx={{ mb: 2 }}>
          <Stack direction={{ xs: "column", md: "row" }} spacing={2}>
            ...
            <TextField label="Nombre del producto" value={product_name} onChange={(e) => setProductName(e.target.value)} fullWidth />
            <TextField label="Cantidad" type="number" value={quantity} onChange={(e) => setQuantity(Number(e.target.value))} sx={{ width: 120 }} />
          </Stack>
          ...
          <Stack direction={{ xs: "column", md: "row" }} spacing={2}>
            ...
            <Button variant="outlined" onClick={() => { setEditId(null); setProductName(""); setQuantity(1); setStatus("en_progreso"); }}>Limpiar</Button>
